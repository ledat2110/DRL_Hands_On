...
def distr_projection (next_distr_v, rewards_v, dones_mask_t, gamma, device="cpu"):
    ...
    for atom in range(N_ATOMS):
        ...
        b_j = (tz_j - V_MAX) / DELTA_Z
        l = np.floor(b_j).astype(np.int64)
        r = np.ceil(b_j).astype(np.int64)
        eq_mask = l == r
        proj_distr[eq_mask, l[eq_mask]] += next_distr[eq_mask, atom]
        ne_mask = l != r
        proj_distr[ne_mask, l[ne_mask]] += next_distr[ne_mask, atom] * (r - b_j)[ne_mask]
        proj_distr[ne_mask, r[ne_mask]] += next_distr[ne_mask, atom] * (b_j - l)[ne_mask]

    if dones_mask.any():
        proj_distr[dones_mask] = 0
        ...
        eq_dones[dones_mask] = eq_mask # dones_mask & eq_mask
        if eq_dones.any():
            proj_distr[eq_dones, l[eq_mask]] = 1.0
        ne_mask = l != r
        ne_dones = dones_mask.copy()
        ne_dones[dones_mask] = ne_mask # dones_mask & ne_mask
        if eq_dones.any():
            proj_distr[ne_dones, l[ne_mask]] = (r - b_j)[ne_mask]
            proj_distr[ne_dones, r[ne_mask]] = (b_j - l)[ne_mask]
...
if __name__ == "__main__":
    ...
    crt_net = model.D4PGCritic(env.observation_space.shape[0], env.action_space.shape[0], N_ATOMS, V_MIN, V_MAX).to(device)
    print(act_net)
    print(crt_net)
    ...
    with drl.tracker.RewardTracker(writer) as tracker:
        with drl.tracker.TBMeanTracker (writer, 10) as tb_tracker:
            while True:
                ...
                proj_distr_v = distr_projection(last_distr_v, rewards_v, dones_mask_v, gamma=GAMMA**REWARD_STESP, device=device)
