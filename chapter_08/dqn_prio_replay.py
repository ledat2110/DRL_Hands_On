...
def calc_loss (batch, batch_weight, net, tgt_net, gamma, device="cpu"):
    ...
    batch_weight_v = torch.tensor(batch_weight).to(device)
    ...
    loss_v = batch_weight_v * loss

    return loss_v.mean(), (loss_v + 1e-5).data.cpu().numpy()
...
if __name__ == "__main__":
    ...
    buffer = dexp.PrioReplayBuffer(params.replay_size, PRIO_REPLAY_ALPHA, BETA_START)
    ...
    while True:
        ...
        beta_tracker.update_beta(frame_idx)
        ...
        if reward is not None:
            ...
            print("%d: done %d games, reward %.3f, eps %.2f, beta %.2f, speed %.2f f/s"%(frame_idx, len(total_reward), m_reward, selector.epsilon, buffer.beta, speed))
            writer.add_scalar("epsilon", selector.epsilon, frame_idx)
            writer.add_scalar("beta", buffer.beta, frame_idx)
            writer.add_scalar("speed", speed, frame_idx)
            writer.add_scalar("reward_100", m_reward, frame_idx)
            writer.add_scalar("reward", reward, frame_idx)

            if best_m_reward is None or best_m_reward < m_reward:
                torch.save(net.state_dict(), params.env_name + "-prio-replay-best_%.0f.dat"%m_reward)
        ...
        loss_t, sample_prios = calc_loss(batch, batch_weights, agent.dqn_model, tgt_net.target_model, params.gamma, device)
        loss_t.backward()
        optimizer.step()
