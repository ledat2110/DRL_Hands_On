...
def calc_loss (batch, net, tgt_net, gamma, device="cpu"):
    ...
    sa_vals = distr_v[range(batch_size), actions_v.data.type(torch.long)]
    ...
    while True:
        ...
        if reward is not None:
            ...
            if best_m_reward is None or best_m_reward < m_reward:
                torch.save(net.state_dict(), params.env_name + "-distribution-best_%.0f.dat"%m_reward)
        ...
        loss_t = calc_loss(batch, net, tgt_net.target_model, params.gamma, device)
        loss_t.backward()
        optimizer.step()
