...
if __name__ == "__main__":
    ...
    net = dqn_extra.NoisyDQN(input_shape, n_actions).to(device)
    ...
    while True:
        ...
        if reward is not None:
            ...
            print("%d: done %d games, reward %.3f, speed %.2f f/s"%(frame_idx, len(total_reward), m_reward, speed))
            writer.add_scalar("speed", speed, frame_idx)
            writer.add_scalar("reward_100", m_reward, frame_idx)
            writer.add_scalar("reward", reward, frame_idx)

            if best_m_reward is None or best_m_reward < m_reward:
                torch.save(net.state_dict(), params.env_name + "-noisy-best_%.0f.dat"%m_reward)
