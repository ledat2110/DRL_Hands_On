...
class InputWrapper (gym.ObservationWrapper):
    ...
    def __init__ (self, *args):
        ...
        self.observation_space = spaces.Box(self.observation(old_space.low), self.observation(old_space.high), dtype=np.float32)
...
def iterate_batches (envs, batch_size: int=BATCH_SIZE):
    ...
    while True:
        ...
        obs, reward, is_done, _ = e.step(action)

        if np.mean(obs) > 0.01:
            batch.append(obs)
        if len(batch) >= batch_size:
            # Normalising input between -1 and 1
            batch_np = np.array(batch, dtype=np.float32) * 2.0 / 255 - 1
