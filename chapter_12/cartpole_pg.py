...
if __name__ == "__main__":
    ...
    agent = drl_lib.agent.PolicyAgent(net, apply_softmax=True)
    exp_source = drl_lib.experience.ExperienceSource(env, agent, steps_count=REWARD_STEPS)
    ...
    for step_idx, exp in enumerate(exp_source):
        ...
        loss_v = entropy_loss_v.item() + loss_policy_v.item()
        optimizer.step()
        ...
        writer.add_scalar("grad_l2", grad_l2, step_idx)
        writer.add_scalar("grad_max", grad_max, step_idx)
        writer.add_scalar("grad_var", grad_var, step_idx)
