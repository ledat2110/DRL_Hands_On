...
from lib import model, common, trpo
...
if __name__ == "__main__":
    ...
    parser.add_argument("--maxkl", default=TRPO_MAX_KL, type=float, help="Maximum KL divergence rate")
    ...
    with drl.tracker.RewardTracker(writer) as tracker:
        with drl.tracker.TBMeanTracker (writer, 10) as tb_tracker:
            for step_idx, exp in enumerate(exp_source):
                ...
                def get_loss ():
                    mu_v = act_net (traj_states_v)
                    logprob_v = drl.common.utils.cal_cont_logprob(mu_v, act_net.logstd, traj_actions_v)
