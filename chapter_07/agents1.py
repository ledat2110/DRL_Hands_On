...
if __name__ == "__main__":
    ...
    selector = rl_lib.actions.ArgmaxActionSelector()
    agent = rl_lib.agent.DQNAgent(net, selector)
    ag_out = agent(state)
    print("dqn agent out", ag_out)

    selector = rl_lib.actions.EpsilonGreedyActionSelector(epsilon=0)
    ...
    selector = rl_lib.actions.ProbabilityActionSelector()
    agent = rl_lib.agent.PolicyAgent(model=net, action_selector=selector, apply_softmax=True)
    ag_out = agent(state)
